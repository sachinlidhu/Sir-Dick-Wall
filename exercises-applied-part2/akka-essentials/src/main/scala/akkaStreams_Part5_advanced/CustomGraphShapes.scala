...
object CustomGraphShapes extends App{
  ...
  //lets start with simple 2*3 shape
  case class Balance2x3 (
                   in0: Inlet[Int],
                   in1: Inlet[Int],
                   out0: Outlet[Int],
                   out1: Outlet[Int],
                   out2: Outlet[Int] // i want to expose these parameters as members thats why we create it as case class
                   ) extends Shape {
    //Inlet[T], Outlet[T]
    //This balance class will have 2 inlets and 3 outlets
  ...
  val balance2x3Impl = GraphDSL.create() {
    implicit builder =>
      ...
      val merge = builder.add(Merge[Int](2)) //as it is 2x3 shape
  ...
  val balance2x3Graph = RunnableGraph.fromGraph(
    GraphDSL.create() { implicit builder =>
      ...
      def createSink(index: Int) = Sink.fold(0)(
        (count: Int, element: Int) => {
          println(s"Received $element, current count is $count")
          count + 1
        }
      )
    ...
    def apply[T](inputCount: Int, outputCount: Int): Graph[BalanceMxN[Int], NotUsed] =
      GraphDSL.create() {
        implicit builder =>
          ...
          val merge = builder.add(Merge[Int](inputCount)) //as it is Mxn shape
