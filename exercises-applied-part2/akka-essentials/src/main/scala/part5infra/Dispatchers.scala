...
object Dispatchers extends App {
  ...
  // method 1 to attaching dispatcher to th actors---------programatic ..in code
  ...
  //method 2 --going through akka.actor.deploymnet in config
  ...
  class DBActor extends Actor with ActorLogging {
    ...
    //if u want to access some special resources then use custom dispacter from config
    //use a dedicated dispatcher for blocking calls
    //context.system.dispatchers.lookup("my-dispatcher")---this is sol 1
    //solution 2 is use router
    override def receive: Receive = {
      case message => Future {
        //wait on a resource
        Thread.sleep(5000)
