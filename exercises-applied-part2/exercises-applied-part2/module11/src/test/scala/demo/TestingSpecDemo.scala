...
class TestingSpecDemo extends FunSpec with Matchers with PropertyChecks with MockFactory {
  ...
  describe ("Retrieving the weather from the weather service") {
    ...
    it("should call getWeather if operational is true---sachin") {
      val mockWS = mock[WeatherService]

      (mockWS.operational _: () => Boolean).expects().returning(true)
      //(mockWS.getWeather _).expects("PDX").returning(Weather(55.0, 0.0, 1012.0))
      (mockWS.getWeather _).expects("SFO").returning(Weather(65.0, 0.3, 1008.0))

      val weather1 = lookupWeather(mockWS, "SFO")

      val tolerance1 = 1e-6

      weather1 should be(defined)
      weather1.get.precip should be(0.3 +- tolerance1)
      weather1.get.pressure should be(1008.0 +- tolerance1)
      weather1.get.temp should be(65.0 +- tolerance1)
